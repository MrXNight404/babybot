/*
]=====> RIZAL MCA <=====[ ]=====> YT Gabuts Zall <=====[ ]=====> 083874570716 <=====[
*/
const {
   WAConnection,
   MessageType,
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:RIZAL MCA\n' 
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6283874570716:+62 838-7457-0716\n' 
            + 'END:VCARD'
const ngonsol = JSON.parse(fs.readFileSync('./settings/Rizal.json'))
const {
    botName,
    ownerName,
    BarBarKey,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT GABUTSZALL CHANNEL'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./Rizal.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./Rizal.json') && baby.loadAuthInfo('./Rizal.json')
	baby.on('connecting', () => {
		start('2', 'Rizal Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Rizal Connected')
	})
...
*/
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 Rizal baik hati`)
				ramlan = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${rizal}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 Rizal baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis3?text=${rizal}&APIKEY=${XteamKey}`)
				baby.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak :*'})
				break
				case 'nulis':
				case 'tulis':
				...
				baby.sendMessage(from, ''+tes+'\n\n_By : ⸸Rizal⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break				
				case 'ninjalogo':
                  if (isBanned) return reply(ind.baned())
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo Rizal & Gans`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Rizal & Hub`)
				reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}gemboktext 11 01 2021 & Rizal dan Nadia`)
					reply(ind.wait())
				...
				if (!isPrem) return reply(ind.premium())
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext Rizal & Gans`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Rizal', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata Rizal`
...
case 'neontext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext Rizal`)
neon = body.slice(10)
reply('「❗」WAIT GANS')
...
case 'blackpink':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink Rizal`)
pink = body.slice(11)
...
*/
                ...
                addKoinUser('6283874570716@s.whatsapp.net', fee)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Rizal`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Rizal baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext Rizal`)
cloud = body.slice(11)
...
> RIZAL MCA
> TERIMIKASI BUAT YANG SUDAH
> HINA SAYA SETIAP HARI
> SAMPE MENTAL SAYA DOWN
> DAN BUAT KAMU SAYA DOAKAN
> SEMOGGA KAMU MENDAPATKAN
> HIDAYAH DAN BERKAH SELALU
> 
